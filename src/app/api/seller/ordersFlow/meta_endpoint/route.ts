...
import { handleOrdersFlow } from "@/handlers/seller/ordersFlow_handler";
...
export async function POST(req: NextRequest) {
  try {
    ...
    try {
      ...
      const err =
        typeof deErr === "object" && deErr !== null && "message" in deErr
          ...
          : {};
      console.error("Orders flow decryption failed:", err.message || deErr);
    ...
    if (parsed.action === "ping" || parsed.action === "PING") {
      ...
      const flowResponse = await handleOrdersFlow(parsed);
      resp = {
    ...
    console.error("Orders flow processing error:", e.message || err);

    const msg = String(e.message || "Unable to process orders flow");
    const status =
